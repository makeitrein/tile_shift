...
export function splitListItem(
  ...
  return function (
    ...
    const checkbox: Node | null =
      listItem.firstChild?.type == rinoCheckboxType
        ? listItem.firstChild
        : null;
    ...
    if (parent.content.size === 0) {
      ...
      if (isEmptyListItem) {
        if (dispatch) {
          ...
          assert(
            all(newListItems1.map((node) => node.type.name === "rinoListItem"))
          );
          assert(
            all(newListItems2.map((node) => node.type.name === "rinoListItem"))
          );
          assert(
            newListItems1.length + newListItems2.length + 1 ===
              oldList.childCount,
      ...
      const nextType =
        $to.pos === $from.end() ? listItem.contentMatchAt(0).defaultType : null;
