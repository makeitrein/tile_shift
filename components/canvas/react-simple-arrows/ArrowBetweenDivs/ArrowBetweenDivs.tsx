...
import {
  ...
  getArrowAnchorPositionForCoordinates,
...
export const ArrowBetweenDivs = ({
  ...
  if (!fromCoordinates && debug) {
    if (debug)
      console.warn(
        `ArrowBetweenDivs: coordinates for from.id ${from.id} are not defined yet`
  ...
  if (!toCoordinates) {
    if (debug)
      console.warn(
        `ArrowBetweenDivs: coordinates for to.id ${to.id} are not defined yet`
      ); // output message if in debug mode
  }
  ...
  const start = getArrowAnchorPositionForCoordinates({
    coordinates: fromCoordinates,
    placement: from.placement,
  });
  const end = getArrowAnchorPositionForCoordinates({
