...
function Panzoom(
  ...
  function setMinMax() {
    if (options.contain) {
      ...
      const parentHeight =
        dims.parent.height - dims.parent.border.top - dims.parent.border.bottom;
  ...
  function zoomToPoint(
    ...
    return zoom(
      toScale,
      { animate: false, ...zoomOptions, focal },
      originalEvent
    );
  }
  ...
  function move(event: PointerEvent) {
    ...
    if (pointers.length > 1) {
      ...
      const toScale = constrainScale((diff * options.step) / 80 + startScale)
        .scale;
